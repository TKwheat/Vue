...
<div>
    ...
    <!-- 编辑分类弹出框 -->
    <el-dialog title="编辑分类" :visible.sync="editCateVisible" width="50%" @close="editCateDialogClose" >
        <!-- 内容主体 -->
        <el-form :model="editCatdData" :rules="editCatRules" ref="editCatRef" label-width="80px">
          <el-form-item label="分类名称"  prop="cat_name">
            <el-input v-model="editCatdData.cat_name"></el-input>
          </el-form-item>
        </el-form>
        <!-- 按钮区 -->
        <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    async editCate (id) {
      ...
      this.editCatdData = res.data
      console.log(this.editCatdData)
    },
    // 编辑分类对话框关闭事件
    editCateDialogClose () {
      // 重置表单
    ...
    editCateInfo () {
      this.$refs.editCatRef.validate(async valid => {
        if (!valid) return
        // 发起修改请求
        const { data: res } = await this.$http.put('categories/' + this.editCatdData.cat_id, { cat_name: this.editCatdData.cat_name })
